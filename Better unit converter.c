...
#include <stdio.h>
//functions to separately convert different types of values
double convertM(double); //meters to feet
double convertG(double); //grams to pounds
double convertC(double); //T in Celsius to T in Farenheit 
...
double convertM(double m) {
    return m*3.2808;
}

double convertG(double g) {
    return g*0.002205;
}

double convertC(double c) {
    return (32.0 + (1.8 * c));
}
